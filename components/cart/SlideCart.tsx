...
export default function SlideCart() {
  const { slideCartHandler, cart, updateCartHandler } = useCart();
  return (
    <div className="slidecart flex fixed top-0 z-50 items-center h-screen w-full">
      ...
      <aside className="bg-white w-1/4 z-50 h-full px-4">
        {cart ? (
          ...
          cart.map((item) => (
            <div
              className="cart-list my-4 border-b flex flex-col py-4"
              key={item.title}
            >
              <div className="top flex items-center w-full justify-between">
                <img src={item.img} alt={item.title} className="w-1/4 mr-6" />
